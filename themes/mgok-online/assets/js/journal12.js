...
if (saveBtn) {
	saveBtn.addEventListener('click', (event) => {
		...
		if (isAdditionalValidate && isMarksValidate) {
			const isAdditionalHaveMarks = validationAdditionalMarks(additionalDates, additionalMarks);
...
function validationAdditionalMarks(additionalDates, markInputs) {
	if (additionalDates.length !== 0 && markInputs.length === 0) {
		Swal.fire(
